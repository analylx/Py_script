...
import datetime
import tkinter
from tkinter import PanedWindow, Frame, Label, Entry, Button
...
# 使窗体最大化,zoomed only enabled in windows
tk.state("zoomed")
# 左右分隔窗体
main_panedwindow = PanedWindow(tk, sashrelief=RAISED, sashwidth=5)
# 协议编辑区窗体
protocol_editor_panedwindow = PanedWindow(
    orient=VERTICAL, sashrelief=RAISED, sashwidth=5)
# 协议导航树
protocols_tree = Treeview()
# 当前网卡的默认网关/ in windows use:os.popen('route print')
default_gateway = "0.0.0.0"


def default_gateway_win():
    default_gateway = [
        a for a in os.popen('route print').readlines() if ' 0.0.0.0 ' in a][0].split()[-3]
    return default_gateway


default_gateway = default_gateway_win()
# 用来终止数据包发送线程的线程事件
...
def create_protocols_tree():
    ...
    # 应用层
    applicatoin_layer_tree_entry = protocols_tree.insert(
        "", 0, "应用层", text="应用层")  # ""表示父节点是根
    http_packet_tree_entry = protocols_tree.insert(
        applicatoin_layer_tree_entry, 1, "HTTP包", text="HTTP包")
    dns_packet_tree_entry = protocols_tree.insert(
        applicatoin_layer_tree_entry, 1, "DNS包", text="DNS包")
    # 传输层
    transfer_layer_tree_entry = protocols_tree.insert("", 1, "传输层", text="传输层")
    tcp_packet_tree_entry = protocols_tree.insert(
        transfer_layer_tree_entry, 0, "TCP包", text="TCP包")
    upd_packet_tree_entry = protocols_tree.insert(
        transfer_layer_tree_entry, 1, "UDP包", text="UDP包")
    # 网络层
    ip_layer_tree_entry = protocols_tree.insert("", 2, "网络层", text="网络层")
    ip_packet_tree_entry = protocols_tree.insert(
        ip_layer_tree_entry, 0, "IP包", text="IP包")
    icmp_packet_tree_entry = protocols_tree.insert(
        ip_layer_tree_entry, 1, "ICMP包", text="ICMP包")
    icmpv6_packet_tree_entry = protocols_tree.insert(
        ip_layer_tree_entry, 3, "ICMPv6包", text="ICMPv6包")
    arp_packet_tree_entry = protocols_tree.insert(
        ip_layer_tree_entry, 2, "ARP包", text="ARP包")
    # 网络接入层
    ether_layer_tree_entry = protocols_tree.insert(
        "", 3, "网络接入层", text="网络接入层")
    mac_frame_tree_entry = protocols_tree.insert(
        ether_layer_tree_entry, 1, "MAC帧", text="MAC帧")
...
def on_click_protocols_tree(event):
    ...
    if selected_item[0] == "MAC帧":
        ...
        create_icmp_sender()
    elif selected_item[0] == "ICMPv6包":
        create_icmpv6_sender()
...
def create_protocol_editor(root, field_names):
    ...
    for field in field_names:
        ...
        entry = Entry(
            row, font=('Courier', '12', 'bold'), state='normal')  # 设置编辑框为等宽字体
...
def create_mac_sender():
    ...
    send_packet_button, reset_button, default_packet_button = create_bottom_buttons(
        protocol_editor_panedwindow)
    # 为"回车键"的Press事件编写事件响应代码，发送MAC帧,绑定事件有参数必须使用lambda event:
    tk.bind('<Return>',
            (lambda event: send_mac_frame(entries, send_packet_button)
             ))  # <Return>代表回车键
    ...
    reset_button.bind('<Button-1>',
                      (lambda event: clear_protocol_editor(entries)))
    # 为"默认值"按钮的单击事件编写事件响应代码，在协议字段编辑框填入MAC帧字段的默认值
    default_packet_button.bind(
        '<Button-1>', (lambda event: create_default_mac_frame(entries)))
...
def create_arp_sender():
    ...
    tk.bind('<Return>',
            (lambda event: send_arp_packet(entries, send_packet_button)
             ))  # <Return>代表回车键
    ...
    default_packet_button.bind(
        '<Button-1>', (lambda event: create_default_arp_packet(entries)))
...
def create_icmp_sender():
    """
    now it is IP sender, will change to ICMP sender indeed.
    :return: None
    """
    # IP帧编辑区
    ...
    # 为"回车键"的Press事件编写事件响应代码，发送ARP包
    tk.bind('<Return>',
            (lambda event: send_icmp_packet(entries, send_packet_button)
             ))  # <Return>代表回车键
    # 为"发送"按钮的单击事件编写事件响应代码，发送ARP包
    send_packet_button.bind(
        '<Button-1>',
    ...
    # 为"默认值"按钮的单击事件编写事件响应代码，在协议字段编辑框填入ARP包字段的默认值
    default_packet_button.bind(
        '<Button-1>', (lambda event: create_default_icmp_packet(entries)))


def create_icmpv6_sender():
    pass
...
def create_default_icmp_packet(entries):
    ...
    entries[0].insert(0, int(default_icmp_packet.version))
    entries[1].insert(0, 5)
    entries[3].insert(0, 20)
    entries[2].insert(0, hex(default_icmp_packet.tos))
    entries[4].insert(0, int(default_icmp_packet.id))
    entries[5].insert(0, int(default_icmp_packet.flags))
    entries[6].insert(0, int(default_icmp_packet.frag))
    entries[7].insert(0, int(default_icmp_packet.ttl))
    entries[8].insert(0, int(default_icmp_packet.proto))
    entries[9]['state'] = NORMAL  # 可操作
    entries[9].insert(0, "单击发送时自动计算")
    entries[9]['state'] = DISABLED  # 不可操作
    # 目标IP地址设成本地默认网关
    entries[11].insert(0, default_gateway)
    default_icmp_packet = ICMP(dst=entries[11].get())  # 可以省略
    entries[10].insert(0, default_icmp_packet.src)


def create_default_icmpv6_packet(entries):
    pass
...
def send_icmp_packet(entries, send_packet_button):
    ...
    if send_packet_button['text'] == '发送':
        ...
        packet_to_send = ICMP(
            ...
            src=ip_src,
            dst=ip_dst)
        packet_to_send = ICMP(raw(packet_to_send))
...
def send_icmpv6_packet(entries, send_packet_button):
    pass
...
def create_ip_sender():
    ...
    # 为"默认值"按钮的单击事件编写事件响应代码，在协议字段编辑框填入ARP包字段的默认值
    default_packet_button.bind(
        '<Button-1>', (lambda event: create_default_ip_packet(entries)))
...
def create_tcp_sender():
    ...
    # 为"回车键"的Press事件编写事件响应代码，发送ARP包
    tk.bind('<Return>',
            (lambda event: send_tcp_packet(entries, send_packet_button)
             ))  # <Return>代表回车键
    # 为"发送"按钮的单击事件编写事件响应代码，发送ARP包
    send_packet_button.bind(
        '<Button-1>',
    ...
    default_packet_button.bind(
        '<Button-1>', (lambda event: create_default_tcp_packet(entries)))
...
def create_udp_sender():
    ...
    # 为"回车键"的Press事件编写事件响应代码，发送ARP包
    tk.bind('<Return>',
            (lambda event: send_udp_packet(entries, send_packet_button)
             ))  # <Return>代表回车键
    # 为"发送"按钮的单击事件编写事件响应代码，发送ARP包
    send_packet_button.bind(
        '<Button-1>',
    ...
    # 为"默认值"按钮的单击事件编写事件响应代码，在协议字段编辑框填入TCP包字段的默认值
    default_packet_button.bind(
        '<Button-1>', (lambda event: create_default_udp_packet(entries)))
...
def create_http_sender():
    ...
    tk.bind('<Return>',
            (lambda event: send_http_packet(entries, send_packet_button)))
    # <Return>代表回车键
    ...
    default_packet_button.bind(
        '<Button-1>', (lambda event: create_default_http_packet(entries)))
...
def send_packet(packet_to_send):
    ...
    while not stop_sending.is_set():
        if isinstance(packet_to_send, Ether):
            sendp(
                packet_to_send, verbose=0)  # verbose=0,不在控制回显'Sent 1 packets'.
        else:
