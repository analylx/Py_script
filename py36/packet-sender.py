...
default_gateway = [a for a in os.popen('route').readlines() if ' 0.0.0.0 ' in a][0].split()[-3]
# 用来终止数据包发送线程的线程事件
...
def send_icmp_packet(entries, send_packet_button):
    ...
    if send_packet_button['text'] == '发送':
        ...
        packet_to_send = ICMP(version=ip_version, ihl=ip_ihl, tos=ip_tos, len=ip_len, id=ip_id,
