# -*- coding:utf-8 -*-
import re
def regresion_cips_so_ip_bfd(sessions,interval=10):
    """
    sessions: int BFD进程数
    interval:int 倒换的间隔
    """
    ...
    r2= re.compile(r"Session up time: 00:")
    ...
    for uptime in up_times:
        ...
        if int(uptime.split(':')[2])< interval and uptime.split(':')[1].find(" 00",0,3)== 0 :
            p_list[n%sessions]+=1
        n += 1
    return p_list

if __name__ == '__main__':
    p_list = regresion_cips_so_ip_bfd(7)
    length = 1
    for p in p_list:
