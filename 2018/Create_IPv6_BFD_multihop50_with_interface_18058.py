...
def do_write(unit,ip_head):
    with open(file_path, 'a') as fi:
        fi.write("""set interfaces xe-ts7/1 unit {0} family inet6 address {1}::58/64
set interfaces xe-ts7/1 unit {0} vlan-id-list {0}
set protocols multihop-bfd ipv6 source {1}::58 destination {1}::56
""".format(unit,ip_head))
