...
def do_write(unit,IP1,IP2,interface1):
    with open(file_path, 'a') as fi:
        fi.write('''set interfaces xe-ts7/2 unit {0} family inet6 address 2102:2:{2}:{1}::58/64
set interfaces xe-ts7/2 unit {0} vlan-id-list {0}
set protocol multihop-bfd ipv6 source 2102:2:{2}:{1}::58 destination 2102:2:{2}:{1}::81 minimum-receive-interval 100 minimum-transmit-interval 100 multiplier 3\n''' .format(unit,IP1,IP2,interface1,interface1.split('.')[0]))
