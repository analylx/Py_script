...
def do_write(unit,IP1,IP2,interface1):
    with open(file_path, 'a') as fi:
        fi.write('''set interfaces xe-xsa/1 unit {0} family inet address 2.{2}.{1}.81/255.255.255.0
set interfaces xe-xsa/1 unit {0} vlan-id-list {0}
set protocol bfd interface xe-xsa/1.{0} destination 2.{2}.{1}.58 minimum-receive-interval 3.33 minimum-transmit-interval 3.33 multiplier 3\n''' .format(unit,IP1,IP2,interface1,interface1.split('.')[0]))
