...
def do_write(unit,IP1,IP2,interface1):
    with open(file_path, 'a') as fi:
        fi.write('''set interfaces xe-xsa/1 unit {0} family inet6 address 2101:2:{2}:{1}::81/64
set interfaces xe-xsa/1 unit {0} vlan-id-list {0}
set protocol multihop-bfd ipv6 source 2101:2:{2}:{1}::81 destination 2101:2:{2}:{1}::58 minimum-receive-interval 100 minimum-transmit-interval 100 multiplier 3\n''' .format(unit,IP1,IP2,interface1,interface1.split('.')[0]))
