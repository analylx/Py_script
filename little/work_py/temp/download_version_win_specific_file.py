...
def copyfile_download(paths, childpath):
    ...
    print "paths in the function copyfile_download:" , paths
    print "childpath in the function copyfile_download:" , childpath #这个childpath前面的逗号两边没空格会被判定缩进不对
    localpath = os.path.split(os.path.realpath(sys.argv[0]))[0]
    print "copyfile_download local path:",localpath
    putpath = path(paths, localpath, replace_sharepath)
    print "copyfile_download putpath：",putpath
    try:
